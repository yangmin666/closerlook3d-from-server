...
def training(config):
    # print("config:",config)
    # assert 1==2
    #config: {'max_epoch': 600, 'batch_size': 8, 'base_learning_rate': 0.02, 'optimizer': 'sgd', 'momentum': 0.98, 
    #'activation_fn': 'relu', 'init': 'xavier', 'weight_decay': 0.001, 'bn_momentum': 0.99, 'bn_eps': 1e-06, 'grad_norm': 100, 
    #'decay_epoch': 1, 'decay_rate': 0.9885531, 'epoch_steps': 500, 'validation_size': 50, 'num_classes': 0, 'num_parts': [], 
    #'in_features_dim': 5, 'bottleneck_ratio': 2, 'depth': 1, 'first_features_dim': 72, 'num_layers': 5, 'global_pooling': 'avg', 
    #'first_subsampling_dl': 0.04, 'density_parameter': 5.0, 'in_radius': 2.0, 'augment_scale_anisotropic': True, 
    #'augment_symmetries': [True, False, False], 'augment_rotation': 'vertical', 'augment_scale_min': 0.7, 'augment_scale_max': 1.3, 
    #'augment_noise': 0.001, 'augment_color': 0.8, 'num_gpus': 2, 'num_threads': 4, 'gpus': [6, 7], 'load_path': None, 
    #'print_freq': 10, 'save_freq': 10, 'val_freq': 10, 'log_dir': 'log/s3dis/pseudogrid_1608125461', 'local_aggreagtion': 
    #'pseudo_grid', 'pospool': {'position_embedding': 'xyz', 'reduction': 'mean', 'output_conv': False}, 
    #'adaptive_weight': {'local_input_feature': 'dp', 'reduction': 'mean', 'shared_channels': 1, 'fc_num': 1, 'weight_softmax': False,
    #'output_conv': False}, 'pointwisemlp': {'local_input_feature': 'dp_fj', 'fc_num': 1, 'reduction': 'max'}, 
    #'pseudo_grid': {'fixed_kernel_points': 'center', 'KP_influence': 'linear', 'KP_extent': 1.0, 'num_kernel_points': 15, 
...
def train_one_epoch(sess, ops, epoch, lr):
    ...
    while True:
        try:
            _, loss, labels_val, segment_loss, weight_loss = sess.run([ops['train_op'],
                                                           ops['loss'],
                                                           ops['tower_labels'],
                                                           ops['segment_loss'],
