...
def parse_option():
    ...
    config.val_freq=1 #自己加的，每10轮测一下
    config.save_freq=1 #自己加的，每10轮保存一次
...
def training(config):
    with tf.Graph().as_default():
        ...
        #下面两行是我加的
        config.num_classes = dataset.num_classes - len(dataset.ignored_labels) 
        ...
        if config.load_path is not None:
            num_epoch_begin_lr=int(config.load_path.split('-')[-1])
...
def val_one_epoch(sess, ops, dataset, validation_probs, val_proportions, epoch):
    ...
    while True:
        try:
            ...
            print('tower_labels[0] = ', tower_labels[0].shape)
            print('tower_probs[0].shape = ', tower_probs[0].shape) #(13170, 21)
    ...
    print("predictions[0].shape:",predictions[0].shape) #(9813, 20)
