...
def training(config):
    with tf.Graph().as_default():
        ...
        for i, igpu in enumerate(config.gpus):
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                with tf.device('/gpu:%d' % (igpu)), tf.name_scope('gpu_%d' % (igpu)) as scope:
                    ...
                    # print('total_loss_contextual = ', losses_contextual)
        ...
        # total_loss = total_loss+total_loss_contextual
...
def val_one_epoch(sess, ops, dataset, validation_probs, validation_probs_contextual, val_proportions, epoch):
        ...
        try:
            ...
            for stacked_probs, stacked_probs_contextual, labels, batches, point_inds, cloud_inds in zip(tower_probs, tower_probs_contextual, tower_labels,
                ...
                for b_i, b in enumerate(batches):
                    ...
                    probs_contextual = stacked_probs_contextual[b]
                    inds = point_inds[b]
                    c_i = cloud_inds[b_i]
                    # Update current probs in whole cloud
                    validation_probs[c_i][inds] = val_smooth * validation_probs[c_i][inds] + (1 - val_smooth) * probs
                    validation_probs_contextual[c_i][inds] = val_smooth * validation_probs_contextual[c_i][inds] + (1 - val_smooth) * probs_contextual
                    # Stack all prediction for this epoch
                    predictions += [probs]
                    predictions_contextual += [probs_contextual]
    ...
    IoUs_contextual, mIoU_contextual = s3dis_subset_metrics(dataset, predictions_contextual, targets, val_proportions)
    vote_IoUs_contextual, vote_mIoU_contextual = s3dis_voting_metrics(dataset, validation_probs_contextual, val_proportions)
...
def val_vote_one_epoch(sess, ops, dataset, epoch, num_votes=20):
    ...
    val_probs_contextual = [np.zeros((l.shape[0], nc_model), dtype=np.float32) for l in dataset.input_labels['validation']]
    ...
    while last_min < num_votes:
        try:
            ...
            if last_min + 1 < new_min:
                ...
                IoUs_contextual, mIoU_contextual = s3dis_voting_metrics(dataset, val_probs_contextual, val_proportions)
                logger.info(f'E{epoch} S{vote_ind} * mIoU {mIoU:.3%}')
                logger.info(f'E{epoch} S{vote_ind} * mIoU {mIoU_contextual:.3%}')
                ...
                if int(np.ceil(new_min)) % 2 == 0:
                    ...
                    IoUs_contextual, mIoU_contextual = s3dis_metrics(dataset, proj_probs_contextual)
