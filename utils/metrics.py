...
def s3dis_subset_metrics(dataset, predictions, targets, val_proportions):
    ...
    mIoU = np.mean(IoUs)
    return IoUs, mIoU
def scannet_subset_metrics(dataset, predictions, targets, val_proportions):
...
def scannet_voting_metrics(dataset, validation_probs, val_proportions):
    Confs = []
    for i_test in range(dataset.num_validation):
        # Insert false columns for ignored labels
        probs = validation_probs[i_test]
...
def scannet_metrics(dataset,proj_probs):
    Confs = []
    for i_test in range(dataset.num_validation):
        # Insert false columns for ignored labels
        for l_ind, label_value in enumerate(dataset.label_values):
